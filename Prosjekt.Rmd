...
#### USA 10Y Bonds dailty rate 

rs <- 5000 # angi sats for rente skalering 

usa10ybond <- read_csv("data/United States 10-Year Bond Yield Historical Data.csv", 
    col_types = cols(Date = col_date(format = "%b %d, %Y")))

  usa10ybond <- usa10ybond %>%
  select(Date, Price) %>%
  filter(Date >= as.Date(cod)) %>%
  mutate(Close = Price * rs)

# Renten er skalert opp 10000 ganger for å få frem endringene legge, til orgianl verdi for den på høyre side?   
  

# cut of day 01 Mars 2013 
...
geom_line(data=usa10ybond, aes(x=Date, y=Close), color='purple')
