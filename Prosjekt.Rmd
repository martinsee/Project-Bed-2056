...
rs <- 1000 # angi sats for rente skalering 
...
daily_osebx$Date <- ymd(daily_osebx$Date)
...
#scale_y_continuous(name = "Close",
#                  sec.axis = sec_axis(~./10000, name = "Interest rate %"))+
#                   ggtitle("Amerikanske markedet") +
#                   labs(fill = "Eiendel")
...
alle <- dowj %>%
  ...
  full_join(daily_osebx, by = "Date") %>% 
  rename(dow_j = "Close.x", nas = "Close.y", oil = "Price.x",  oil_sk = "Close.x.x",  sp_500 = "Close.y.y", bitcoin = "Close.x.x.x", usa_3m = "Price.y", usa_3m_sk = "Close.y.y.y") %>% 
  na.omit()
    

alle_long  <- alle %>% 
    select(Date, oil_sk, sp_500, bitcoin, usa_3m_sk, OSEBX) %>%
    gather(key= "asset", value = "Close", -Date)

ggplot(alle_long, aes(x=Date, y=Close, col=asset ))+
  geom_line()+
  scale_y_continuous(name = "Close",
                  sec.axis = sec_axis(~./1000, name = "Interest rate %"))+
                   ggtitle("Amerikanske markedet") +
                   labs(fill = "Eiendel")
...
Korrelasjons i perioden 01.Mars 3013 - Idag 
...
alle_cor  <- alle %>% 
    select(oil, sp_500, bitcoin, usa_3m, OSEBX)
