...
Vi ønsker å undersøke om prisutviklingen på bitcoin er relatert til kursene på verdens aksjemarkeder. Vi har hentet daglig data for bitcoin i USD (BTC/USD), S&P500 og Shanghai Composite så langt bak i tid som mulig. BTC kan kjøpes og selges hele uka, også i helgene, mens børsnoterte aksjer kun handles mandag-fredag måtte vi vurdere om vi kunne kutte ut helge-observasjonene til bitcoin. Vi har beregnet gjennomsnitt for ukedager og helgedager og foretatt en t-test for å finne ut om forskjellene er statistisk signifikante. En alternativ løsning kunne vært å omgjøre prisene og aksjekursene til ukentlig data. Datasettene er lastet ned fra investing.com i form av csv-filer.

Vi har valgt ut den amerikanske indeksen S&P 500 og den kinesiske indeksen shanghai Composite. Disse er vide indekser som favner om mange bransjer, vi diversifiserer dermed bort noe usystematisk risiko som ville gitt bransjemessig støy på dataen. Hensikten er at indeksene skal representere landets akjsemarkedet best mulig.

Vi vil undersøke korrelasjonen mellom bitcoin og det amerikanske aksjemarkedet, og sammenligne det med korrelasjon mellom bitcoin og det kinesiske aksjemarkedet. I vår analyse ønsker vi også å inkludere viktige innsatsfaktorer i øknomien, og vi har dermed inkludert oljeprisen (WTI) og kapital representert med statobligasjonrentene på de obligasjonene med kortest løpetid, USA 3 month treasury bonds og Kinesiske 1 Year bonds. Disse to rentene blir brukt som mål på renten for risikofri kapital i sine respektive marked. Endringe i risikofri rente er viktig for finansmarkedne fordi det inngår i kalkulasjon av avkasningkravet WACC. Hvis renten går opp må avkastningskravet også gå opp. (Langli 2016)   
...
Grafen til BTC viser det ikke, men ved et kall på summary() av dataen ser vi at minimumsprisen på bitcoin er 0. Ved filtrering for 0 viser det seg at datasettet har prisen 0 fire sammenhengende dager. Vi mener at verdien 0 ikke kan være korrekt og representerer manglende data. Datasettet filtreres slik at alle verdier er høyere enn 0. 
...
###Undersøker forskjeller mellom ukedager og helgedager:

```{r}

uked <- c("mandag", "tirsdag", "onsdag", "torsdag", "fredag")
helg <- c("lørdag", "søndag")
...
Stolpediagrammet viser at forskjellene er minimale, men for sikkerhets skyld foretar vi en t-test for å teste om gjenomsnittene for ukedager og helg er signifikant forskjellig. 

Nullhypotese: Ingen forskjell i gjennomsnitt for ukedager og helger. 
Forutsetter lik varians.

p-verdien fra testen er 0.97, noe som er ekstremt høyt. Forskjellene i gjennomsnittene er dermed ikke signifikante fra null, uansett hvilket signifikansnivå vi velger.
Ettersom forskjellene ikke er signifikante har vi valgt å fjerne observasjoner for lørdager og søndager slik at vi kan "matche" datasettet til aksjeindeksene.
...
Summary() av datasettene for aksjeindeksene viser ingen manglende verdier eller nullverdier. Dataen kobles sammen med inner_join på dato som er felles for alle datasett. Inner_join matcher bare dato hvor alle datasettene har verdier. I tillegg omgjør vi datasettet fra "wide" til "long". Hver dato har nå to av den samme observasjonen av BTC matchet med hver av indeksene i kolonnen "index". Dataen strekker seg fra februar 2012 til november 2018. Det sammenslåtte datasettet kaller vi 'ukedager'. 
...
**For S&P 500:** Vi ser at variablene er relaterte, men at forholdet ikke er lineært. En lineær modell vil derfor ikke passe for tidsintervallet.  Vi kan tranformere observasjonene for bitcoin og lage en lineær modell for S&P som utrykk av logaritmeverdiene av bitcoin. I enkelte ggplot-grafer er en svært lav alpha benyttet for å vise den ekstreme overplottingen i nedre venstre hjørne.

**For shanghai Composite:** Ser vi at variablene ikke er relatert i like stor grad.Punktene er veldig spredt ut og ser ikke ut til å følge verken en linæer eller ikke-lineær trend. Shanghai-indeksen har stor variasjon for lave verdier av bitcoin. Midtveis i dataen er det en periode hvor de ser ut til å utvikle seg i samme retning, dette er sannsynligvis en periode i 2014 hvor begge to gikk mye opp samtidig. 
...
Når vi transformerer bitcoin-prisen til log(BTC) kan se en sterk lineær sammenheng. Styrken i forholdet er lik korrelasjonskoeffisienten, som er 0.94.
Positiv korrelasjon innebærer at når den ene variabelen øker, øker også den andre. Korrelasjonen er svært sterk og ikke langt unna perfekt. Ved perfekt korrelasjon er koeffisienten?? lik 1.

**For Shanghai:**
Transformasjonen gir en bedre fit til deler av observasjonene, men ikke det helhetlige forholdet. Forholdet er fremdeles ikke tydelig. En stor mengde observasjoner viser til store endringer i den kinesiske indeksen for de samme verdiene av log(BTC). Observasjonene svekker forholdet for den resterende dataen som følger trenden. korrelasjonskoeffisienten er 0.43. Det er en moderat positiv korrelasjon, men langt svakere enn forholdet mellom BTC og den amerikanske indeksen.
...
**S&P**: beta1 (slope) er 192, for hver økning i prediktorvariabelen øker responsvariabelen med 192. R-squared er 0.89. 89% av variasjon i S&P 500 kan forklares av variasjon i log(BTC), noe som er svært høyt. For lineære modeller med kun en uavhengig variabel er verdien lik korrelasjonskoeffesienten opphøyet i 2. 

**Shanghai**: Modellen viser en beta1 på 126. R-squared er svært lav, 0.182, noe som betyr at under 20% av variasjon i den avhengige variabelen kan forklares av variasjon i log(BTC). Det indikerer en lav prediksjonsverdi og dårlig fit.

Begge indeksene har positive regresjonslinjer. Vi ser at forholdet er sterkere for S&P 500 enn Shanghaiangai Composite siden punktene ligger tettere rundt regresjonslinja. Det innebærer at summen av kvadratene blir mindre og at modellen har en bedre fit.
...
### Vi lastere inn flere datasett, endrer kolonne til dato. Så reduserer vi datasettet ved å selektere de kolonnene vi trenger og endrer navn, begge handlingene gjør vi gjennom tidyverse sin pipe funksjon slik at det blir mer konsist og letter å forså koden. 
...
En multippel regresjonsanalyse av S&P 500 med amerikansk oljepris, rente på 3-måneders amerikanske statsobligasjoner og transformerte verdier av bitcoin som uavhengige variabler. R-squared for modellen er 0.96, dette er høyere enn ved bitcoin som eneste faktor. Når vi legger til olje og statsobligasjoner øker prediksjonsverdien til modellen med 0.07 (0.96-0.89).
...
Svakheten i dataen skal vi illustrere videre i oppgaven ved å analysere samme data, men over ulike tidShanghaioisonter. Bitcoin dataen er over en relativ kort tidShanghaiorisont 2012-2018, og hele perioden sammenfaller med bullmarket i USA etter finanaskrisen, det kan gjøre at dataen kan får en urelatert positiv korrelasjon.   
...
For å vurdere om disse dataen er reliable har vi tatt ut bitcoin slik at vi kan øke tidShanghaiorisonten vår fra 2012-2018 til 2001-2018 
...
Vi ser her at korrelasjonene er mye svakere når tidShanghaiorisonten øker.  Differansen i korrelasjon mellom dataen har vi samlet i en differanse korrelasjon matrix.  
...
# Trekker fra verdiene i kort tidShanghaiorisont matrixen med lang tidShanghaiorisont matrixen for å få en differnase matrix
...
##Oppsummering:
S&P 500 er sterkt korrelert med BTC. Transformering av bitcoinprisen til logaritmeverdier viser en sterk og positiv korrelasjon. Regresjonsmodellen viser at hele 89% av variasjon i indeksen S&P 500 kan forklares av variasjon i log(BTC). Legger vi til oljepris og obligasjonsrente øker R-squared til 0.96. 
For den kinesiske Shanghai-børsen er ikke modellen like god. Med kun log(BTC) som uavhengig variabel fikk regresjonsmodellen en R-squared på 0.18. Inkludert oljepris og obligasjonsrente økte prediksjonsverdien til 0.62, en stor forbedring. 
...
Utfordringen i vår analyse er tidShanghaiorisont. Datasette til bitcoin strekker seg kun tilbake til 2012, fordi det var da bitcoin startet å bli handlet i organiserte marked. Sterk korrelasjon er ikke en indikator på årsakssammenhen eller kausalitet. Uansett hvor sterk korrelasjonen er, kan den være helt tilfeldig. Det tar ikke modellene høyde for.
